...
- **遊樂路線**：5 條建議動線（首推路線取自《亮點100新玩法更新內容_0828》的親子二日遊），
  路線之間可重疊，且**不綁定走訪順序**。
- **站點不分類型**：28 站到現場即可打卡，一站＝一個行程打卡任務。
  消費留在地由「優惠券需消費滿 300 元」把關，打卡本身不設消費門檻。
- **會員等級**（`LEVELS`，文案取自客戶 2026-09-11 規則表）：

  | 等級 | 名稱 | 升級條件 | 點數 | 累積 |
  | --- | --- | --- | --- | --- |
  | 一 | 啟程會員 | 完成會員註冊 | 250 | 250 |
  | 二 | 探索會員 | 任意 2 站打卡（`LEVEL_TWO_CHECKINS`） | +250 | 500 |
  | 三 | 達成會員 | 指定站全部打卡（`DESIGNATED_SPOT_IDS`） | +500 | 1,000 |

  點數只在升級時發放。指定站目前＝首推路線 10 站，改 `DESIGNATED_SPOT_IDS` 一行即可換。
