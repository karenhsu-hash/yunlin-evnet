...
| `/checkin` | 集章頁 | 掃碼／定位兩種方式 → 經緯度比對 → 綁定會員去重 → 觸發自動發券 | 是 |
| `/member` | 我的護照 | 護照個資頁（持有人／護照號碼／效期）＋ 印章頁／我的券夾／使用紀錄 | 是 |
...
- **遊樂路線**：5 條建議動線（首推路線取自《亮點100新玩法更新內容_0828》的親子二日遊），
  路線之間可重疊，且**不綁定走訪順序**。
- **站點兩型**：體驗站 15 處（需現場消費滿 300 元）＋ 亮點站 13 處（拍照打卡即可），共 28 組 QR。
  名稱、說明與配色集中在 `useCampaign.ts` 的 `SPOT_KIND`，要改名只動這一個物件。
- **三段任務**：一個體驗站 ＋ 一個亮點站為一組，依序解鎖 250 / 250 / 500，每人上限 1,000 元。
...
app/
  ...
  composables/
    useCampaign.ts          # 站點資料、SPOT_KIND、ROUTES、活動參數、三段任務規則引擎
...
| 體驗站 | `#C4402E` | 兩色 CVD ΔE 7.9（落在 6–8 下限帶） |
| 亮點站 | `#2E7D53` | 故一律搭配圖例＋直接標籤等次要編碼 |
